...
class WecomSmartSheetDatasourceInitializerTest {
  ...
  @Test
  void installsTheApiOwnedSheetAsTheEnabledBusinessDatasource() throws Exception {
    ...
    ArgumentCaptor<String> disableSql = ArgumentCaptor.forClass(String.class);
    verify(jdbcTemplate).update(disableSql.capture());
    assertThat(disableSql.getValue()).contains("UPDATE datasources", "is_enabled = 0");
    ArgumentCaptor<String> sql = ArgumentCaptor.forClass(String.class);
    @SuppressWarnings("unchecked")
    ArgumentCaptor<Object[]> arguments = ArgumentCaptor.forClass(Object[].class);
    verify(jdbcTemplate, times(15)).update(sql.capture(), arguments.capture());
    assertThat(sql.getAllValues().get(0)).contains("UPDATE datasource_field_mappings", "is_enabled = 0");
    assertThat(arguments.getAllValues().get(0)).containsExactly("sheet-1");
    assertThat(sql.getAllValues().get(1)).contains("INSERT INTO datasources", "ON DUPLICATE KEY UPDATE");
    assertThat(arguments.getAllValues().get(1)).contains("doc-1", "sheet-1", "PrivateDomainAssistant-API");
    assertThat(arguments.getAllValues().subList(2, 15))
        .extracting(values -> List.of(values).toString())
        .containsExactlyInAnyOrder(
            "[sheet-1, 联系方式, phone]",
            "[sheet-1, 备注称呼, nickname]",
            "[sheet-1, 客资类型, leadType]",
            "[sheet-1, 客户阶段, customerStage]",
            "[sheet-1, 客户关注点, bodyConcerns]",
            "[sheet-1, 客户B档案, customerProfileSummary]",
            "[sheet-1, 跟进记录, followupNotes]",
            "[sheet-1, 备注, internalNote]",
            "[sheet-1, 第一次追踪捕捉, firstTrackingCapture]",
            "[sheet-1, 第二次追踪捕捉, secondTrackingCapture]",
            "[sheet-1, 第三次追踪捕捉, thirdTrackingCapture]",
            "[sheet-1, 下次跟进方向, nextFollowupDir]",
            "[sheet-1, 下次跟进时间, nextFollowupAt]");
  ...
  @Test
  void repositoryReadsTheConfiguredDocumentAsTheOnlyEnabledClientTarget() throws Exception {
    ...
    jdbcTemplate.update("""
        ...
        VALUES ('old', 'old-document', 'old-sheet', 'old target', 1, 'SYSTEM'),
               ('old-same-table', 'old-document', 'sheet-1', 'old same-table target', 1, 'SYSTEM')
        """);
    ...
    SheetSource source = new DatasourceAdminRepository(jdbcTemplate).defaultWriteSource().orElseThrow();
    assertThat(source.sheetId()).isEqualTo("doc-1");
    assertThat(source.sourceTable()).isEqualTo("sheet-1");
    assertThat(new DatasourceAdminRepository(jdbcTemplate).enabledSources()).containsExactly(source);
